#!/bin/bash
# =============================================================================
# process_sample_mt_wrapper.sh
# Metatranscriptomics pipeline — rRNA removal + DIAMOND blastx.
# Same as v2 MG wrapper but adds ribodetector rRNA filtering after QC.
#
# Required env vars:
...
TOTAL_READS_RAW=$(( $(wc -l < "${TRIMMED}") / 4 ))
STEP2_END=$(date +%s)
echo "  Reads after trimming: ${TOTAL_READS_RAW}"
echo "  QC time: $(( STEP2_END - STEP2_START ))s"

# =============================================================================
# STEP 2b: rRNA removal (metatranscriptomics-specific)
# =============================================================================
echo ""
STEP2B_START=$(date +%s)
echo "=== [${ACCESSION}] Step 2b: rRNA removal ==="

MRNA="${WORK_DIR}/${ACCESSION}_mrna.fastq"

RRNA_OK=0
# Try GPU first (4× faster)
if command -v ribodetector &>/dev/null; then
    echo "  Trying ribodetector GPU mode..."
    export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
    if ribodetector -d 0 -m 4 -i "${TRIMMED}" -o "${MRNA}" --len 100 --chunk_size 32 2>&1 | tail -5; then
        if [[ -s "${MRNA}" ]]; then
            echo "  GPU mode succeeded"
            RRNA_OK=1
        fi
    fi
fi
# Fall back to CPU
if [[ ${RRNA_OK} -eq 0 ]] && command -v ribodetector_cpu &>/dev/null; then
    echo "  Using ribodetector CPU mode..."
...
if [[ ${RRNA_OK} -eq 0 ]]; then
    echo "  WARNING: ribodetector not found — skipping rRNA removal"
    cp "${TRIMMED}" "${MRNA}"
fi

rm -f "${TRIMMED}"

TOTAL_READS=$(( $(wc -l < "${MRNA}") / 4 ))
RRNA_PCT=$(( (TOTAL_READS_RAW - TOTAL_READS) * 100 / (TOTAL_READS_RAW + 1) ))
STEP2B_END=$(date +%s)
echo "  mRNA reads: ${TOTAL_READS} (removed ${RRNA_PCT}% rRNA)"
echo "  rRNA removal time: $(( STEP2B_END - STEP2B_START ))s"

# Use mRNA reads for DIAMOND
TRIMMED="${MRNA}"
...
{
    echo -e "accession\tlayout\ttotal_reads_raw\tmrna_reads\trrna_pct\tunified_hits"
    echo -e "${ACCESSION}\t${LAYOUT}\t${TOTAL_READS_RAW}\t${TOTAL_READS}\t${RRNA_PCT}\t${TOTAL_HITS}"
} > "${STATS_FILE}"

STEP_END=$(date +%s)
echo "  Stats saved to: ${STATS_FILE}"
echo ""
echo "=== [${ACCESSION}] MT Pipeline complete ==="
echo "  Raw reads (trimmed):   ${TOTAL_READS_RAW}"
echo "  mRNA reads (post-rRNA): ${TOTAL_READS} (${RRNA_PCT}% rRNA removed)"
echo "  Unified hits:          ${TOTAL_HITS}"
echo "  Download: $(( STEP1_END - STEP1_START ))s | QC: $(( STEP2_END - STEP2_START ))s | rRNA: $(( STEP2B_END - STEP2B_START ))s | DIAMOND: $(( STEP3_END - STEP3_START ))s"
echo "STEP_TIMINGS:download=$(( STEP1_END - STEP1_START )),qc=$(( STEP2_END - STEP2_START )),rrna=$(( STEP2B_END - STEP2B_START )),diamond=$(( STEP3_END - STEP3_START ))"
